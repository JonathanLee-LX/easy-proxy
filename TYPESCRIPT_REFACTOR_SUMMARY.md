...
### 优化内容
- ✅ **源代码清理**: core目录现在只保留TypeScript源文件（.ts）
- ✅ **编译产物分离**: 所有编译产物（.js、.d.ts、.map）输出到dist目录
- ✅ **版本控制优化**: 更新.gitignore，编译产物不再纳入版本控制
- ✅ **引用路径更新**: 所有模块引用从`./core/`改为`./dist/`
...
### 优势
1. **源代码更清晰** - core目录职责单一，只包含源码
2. **减小仓库体积** - 编译产物不纳入版本控制
3. **符合最佳实践** - 源码和构建产物分离
4. **便于CI/CD** - 构建过程更加标准化
